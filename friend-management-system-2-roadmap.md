...
- 分数默认下限为 0，不设置上限。
- 需要支持从旧版 FMS 的 `.xlsx` 文件导入历史数据，导入规则参考 `FMS/FMS/Lib/Core.cs` 的 `public void Import(string path)`。
- 前端框架使用 Vue。

这意味着第一版的技术路线应尽量轻：前端应用 + PWA 缓存 + IndexedDB 本地数据库 + JSON 备份导入导出 + 旧 FMS xlsx 迁移导入。
...
- 支持导出 JSON 备份。
- 支持从 JSON 备份恢复数据。
- 支持从旧版 FMS `.xlsx` 文件导入人员、历史榜单、分数、名次和标题。
...
- VueUse 提供常用组合式工具函数。
- SheetJS/xlsx 或同类浏览器端 Excel 解析库，用于导入旧 FMS `.xlsx`。
...
- `db/`：Dexie 表结构、迁移、索引。
- `types/`：核心数据类型。
- `utils/`：排序、日期、导入导出、Excel 解析、ID 生成。
...
- 最近创建的榜单。
- 最近编辑的榜单。
- 快捷入口：添加人员、新建榜单、导出备份、导入旧 FMS xlsx。
...
- 选择旧版 FMS 导出的 `.xlsx` 文件。
- 在浏览器本地解析文件，不上传到服务器。
- 导入前预览：识别到的人员数量、榜单数量、预计条目数量。
- 按旧 FMS 的三列表格块格式读取每一期榜单。
- 将旧 FMS 的数字日期转换为 FMS2 榜单日期。
- 将旧 FMS 每期标题导入为 FMS2 榜单标题，标题缺失时允许为空。
- 将旧 FMS 的姓名导入为人员。
- 将旧 FMS 的分数导入为 `leaderboard_entries.scoreSnapshot`。
- 将旧 FMS 的行号名次导入为 `leaderboard_entries.rank`。
- 导入后重新计算 `movement`、`previousRank`、`rankDelta` 和出榜条目。
...
| note | string | 备注，可为空 |
| boardDate | string \| null | 榜单对应日期，可由旧 FMS 数字日期转换得到 |
| legacyDigitalDate | number \| null | 旧 FMS 的 `yyyyMMdd` 数字日期，非导入数据可为空 |
...
- 分数必须大于等于 `0`。
- 不设置分数上限。
- 出榜对象的 `scoreSnapshot` 可以为 `null`，因为它不参与本期排名。
...
旧版 FMS 的导入逻辑参考 [Core.cs](/home/xq123/Documents/git/fms2/FMS/FMS/Lib/Core.cs:331) 中的 `Import(string path)`，文件格式可以按以下规则理解：

- 只读取第一个工作表。
- 每一期榜单占用 3 列，列起点依次为 `0, 3, 6, ...`。
- 每个三列块的第 1 列是姓名，第 2 列是分数，第 3 列通常作为空白间隔。
- 第 0 行的块起点单元格是数字日期，例如 `20240601`。
- 从第 1 行开始读取榜单条目：块起点列为姓名，后一列为分数。
- 如果某一行只有块起点列有内容、分数列为空，旧 FMS 将其作为该期榜单标题。
- 旧 FMS 预览会先扫描所有块，得到全量姓名列表和全量数字日期列表。
- 旧 FMS 导入时会先为「每个姓名 x 每个日期」初始化空项目，再把 xlsx 中出现的姓名和分数填进去。

映射到 FMS2 时建议：

- 每个数字日期创建一条 `leaderboards`。
- `leaderboards.legacyDigitalDate` 保存旧数字日期。
- `leaderboards.boardDate` 由数字日期转换为 `YYYY-MM-DD`。
- xlsx 标题行映射到 `leaderboards.title`，没有标题则保存空字符串。
- 全量姓名导入为 `people`。
- 每个有姓名和分数的单元格行创建一条 `leaderboard_entries`，`includedInRanking = true`。
- 分数写入 `leaderboard_entries.scoreSnapshot`。
- 名次优先沿用 xlsx 行号，即旧 FMS 的 `item.Rank = y`，避免迁移时改变历史榜单。
- 导入完成后按日期顺序计算 `previousRank`、`rankDelta`、`movement`。
- 如果上一期在榜、本期 xlsx 中没有该人员，则在本期自动补一条 `movement = "out"` 的出榜条目。
- 分数小于 0 的单元格应作为导入错误提示；分数没有上限。

导入模式建议：

- 覆盖导入：清空当前 FMS2 数据，再导入旧 FMS xlsx。MVP 优先实现。
- 追加导入：保留当前数据，把 xlsx 数据追加进来。第二阶段再做。
- 预览导入：只解析文件并展示统计，不写入 IndexedDB。建议 MVP 就做，降低误操作风险。
...
- 分数小数位数：是否允许小数，以及最多保留几位。
- 第一份榜单默认分数。
- 榜单是否默认包含所有未归档人员。
- 标题为空时 UI 显示「未命名榜单」还是日期。
- 删除人员时是否保留历史榜单快照，或只允许归档。
- 旧 FMS xlsx 导入时是覆盖当前数据，还是追加到当前数据。
...
- 导入前有确认提示。
- JSON 结构版本不匹配时给出提示。
- 旧 FMS xlsx 能识别人员数量、榜单数量和条目数量。
- 旧 FMS xlsx 能导入标题、日期、分数、名次。
- 旧 FMS xlsx 导入后能生成初次进榜、回榜、排名变化和出榜条目。
...
- 旧 FMS xlsx 导入后，分数只写入 `leaderboard_entries.scoreSnapshot`。
- 旧 FMS xlsx 导入后，行号名次能保留为 `leaderboard_entries.rank`。
- 删除标签后人员列表仍稳定。
...
| 旧 FMS xlsx 格式不符合预期 | 导入失败或数据错位 | 导入前预览，校验三列块、日期、姓名、分数 |
| 旧 FMS xlsx 行号名次和分数排序不一致 | 历史榜单被错误重排 | 迁移导入默认保留行号名次，不自动重排 |
...
13. 实现 JSON 导出和覆盖导入。
14. 实现旧 FMS xlsx 解析、预览和覆盖导入。
15. 补充排序、快照、变动计算、JSON 导入导出、旧 FMS xlsx 导入测试。
